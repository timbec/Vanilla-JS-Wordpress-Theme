   ...
   static escapeHTML(raw) {
    ...
    static getContentMarkup( content ) {
        ...
        if ( 'page' == content.type ) {
            contentMarkup += `<p class="footer">Last Updated ${lastModified}</p>`;
        }
        contentMarkup += '</div>';
    ...
    /**
     * getExcerptMarkup - Get the markup for excerpt content
     *
     * @param  {Object} excerpt Individual excerpt from the API
     * @return {Object} Content markup with content
     */
    static getExcerptMarkup( content ) {

        let excerptEl = document.createElement( 'div' ),
            excerptMarkup = '',
            date = Helpers.formatDate( content.date ),
            lastModified = Helpers.formatDate( content.modified ),
            author = content._embedded.author[ 0 ].name,
        ...
        if ( 'page' == content.type ) {
            excerptMarkup += `<p class="footer">Last Updated ${lastModified}</p>`;
        }
        excerptMarkup += '</div>';
        excerptEl.innerHTML = excerptMarkup;
    ...
    /**
     * renderExcerpt - Renders content to Page
     *
     * @param  {Object} content Post or Page content object
     * @param {String} titleTag h1, h2, etc for HTML header tag to use
     * @param {Boolean} addLink Whether to display link in title
     * @return {void} Not meant to return
     */
    static renderExcerpt( content, titleTag = 'h1', addLink = false ) {

        const articleEl = document.createElement( 'article' ),
            titleEl = Helpers.getTitleMarkup( content, titleTag, addLink ),
            contentEl = Helpers.getExcerptMarkup( content );
    ...
    /**
     * clearPages - Clears contents on pages
     *
     */
    static clearMenu() {
